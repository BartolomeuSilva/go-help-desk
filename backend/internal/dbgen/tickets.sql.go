...
const updateTicketAIState = `-- name: UpdateTicketAIState :exec
UPDATE tickets
SET ai_active = $2, ai_transferred = $3, updated_at = $4
WHERE id = $1
`

type UpdateTicketAIStateParams struct {
	ID            uuid.UUID `json:"id"`
	AiActive      bool      `json:"ai_active"`
	AiTransferred bool      `json:"ai_transferred"`
	UpdatedAt     time.Time `json:"updated_at"`
...
func (q *Queries) UpdateTicketAIState(ctx context.Context, arg UpdateTicketAIStateParams) error {
	_, err := q.db.ExecContext(ctx, updateTicketAIState,
		arg.ID,
		arg.AiActive,
		arg.AiTransferred,
		arg.UpdatedAt,
