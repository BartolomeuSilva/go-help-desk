...
func (s *Server) handleAddReply(_ context.Context, req mcpgo.CallToolRequest) (*mcpgo.CallToolResult, error) {
	...
	reply, err := s.tickets.AddReply(context.Background(), tid, body, isInternal, notifyRequester, attachmentsJSON, actor, reopenWindowDays, correlationID, "web", nil, false)
	if err != nil {
		return errResult(err.Error())
	}
